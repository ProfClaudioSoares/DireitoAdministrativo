...
export default function StudioPage() {
  ...
  // Trava dos 53px com a MESMA medição do renderizador (§4). Só quando as fontes
  // estão prontas; sem elas, cai no limite bruto de caracteres.
  const bodyFit = useMemo(() => {
    if (!selected?.body) return null
    if (fontsReady) return checkBodyFit(selected.body, TYPE.bodyMin, CONTENT.width)
    const over = selected.body.split('\n').some((l) => l.length > BODY_MAX_CHARS_PER_LINE)
    return over ? { ok: false, overflow: true, message: 'Este slide virou dois. Divida.' } : { ok: true, overflow: false }
  }, [selected?.body, fontsReady])
      ...
      <aside className="border-l border-grey-dark/40 overflow-auto p-6 flex flex-col gap-5">
        {selected && (
          <>
            ...
            <Field label="Rótulo" value={selected.eyebrow ?? ''} onChange={(v) => updateSlide(selected.id, { eyebrow: v })} />
            <Field label="Título" value={selected.title ?? ''} onChange={(v) => updateSlide(selected.id, { title: v })} />
            ...
            <div>
              ...
              <textarea
                ...
                className={`mt-2 w-full bg-transparent border rounded px-3 py-2 outline-none ${
                  bodyFit && bodyFit.overflow ? 'border-amber' : 'border-grey-dark focus:border-amber'
                }`}
              />
              {bodyFit?.overflow && <p className="text-amber text-sm mt-1">{bodyFit.message}</p>}
            </div>

            <Field label="Citação" value={selected.citation ?? ''} onChange={(v) => updateSlide(selected.id, { citation: v })} />
